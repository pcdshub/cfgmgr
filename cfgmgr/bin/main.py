"""
`cfgmgr` is the top-level command for accessing various subcommands.
...
def _try_import(module):
    ...
    return importlib.import_module(relative_module, 'cfgmgr.bin')
...
def _build_commands():
    ...
    if unavailable:
        ...
        for module, ex in unavailable:
            DESCRIPTION += (
                f'\nWARNING: "cfgmgr {module}" is unavailable due to:'
...
def main():
    ...
    top_parser.add_argument(
        '--version', '-V',
        action='version',
        version=cfgmgr.__version__,
        help="Show the cfgmgr version number and exit."
    )
    ...
    logger = logging.getLogger('cfgmgr')
